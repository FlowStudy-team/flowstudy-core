package com.flowstudy.core.module.tutorial;
...
@ActiveProfiles("test")
class TutorialControllerIntegrationTests {
    ...
    void seedDatabase() {
        ...
        jdbcTemplate.update("""
                INSERT INTO fs_blog
                    (id, tutorial_id, title, content_md, sort_order, estimated_minutes, status, deleted)
                VALUES
                    (10, 1, '线程池基础', '## 线程池基础
正文', 1, 15, 'PUBLISHED', 0),
                    (11, 1, '任务调度', '## 任务调度
正文', 2, 10, 'PUBLISHED', 0),
        ...
        jdbcTemplate.update("""
                INSERT INTO fs_problem
                    (id, blog_id, title, description_md, difficulty, status, sort_order, deleted)
                VALUES
    ...
    @Test
    void listPublishedtutorialsWithoutAuthentication() throws Exception {
        mockMvc.perform(get("/api/v1/tutorials?page=1&size=10&keyword=Java"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(0))
                .andExpect(jsonPath("$.data.total").value(1))
                .andExpect(jsonPath("$.data.records", hasSize(1)))
                .andExpect(jsonPath("$.data.records[0].id").value(1))
                .andExpect(jsonPath("$.data.records[0].blogCount").value(2))
                .andExpect(jsonPath("$.data.records[0].problemCount").value(2))
                .andExpect(jsonPath("$.data.records[0].authorName").value("Admin"));
    }

    @Test
    void getTutorialDetailWithblogs() throws Exception {
        mockMvc.perform(get("/api/v1/tutorials/1"))
                ...
                .andExpect(jsonPath("$.data.blogs", hasSize(2)))
                .andExpect(jsonPath("$.data.blogs[0].id").value(10))
                .andExpect(jsonPath("$.data.blogs[0].problemIds[0]").value("100"));
    }

    @Test
    void getTutorialblogsAndBlogDetail() throws Exception {
        mockMvc.perform(get("/api/v1/tutorials/1/blogs"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data", hasSize(2)))
                .andExpect(jsonPath("$.data[0].id").value(10));

        mockMvc.perform(get("/api/v1/blogs/10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(10))
                .andExpect(jsonPath("$.data.tutorialId").value(1))
                ...
                .andExpect(jsonPath("$.data.prevBlogId").doesNotExist())
                .andExpect(jsonPath("$.data.nextBlogId").value(11));
    }

    @Test
    void rejectMissingOrUnpublishedContent() throws Exception {
        mockMvc.perform(get("/api/v1/tutorials/2"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value(42000));

        mockMvc.perform(get("/api/v1/blogs/20"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.code").value(42000));
